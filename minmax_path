...
struct mm_solver{
  void init(int i_length, int i_num){
    ...
    std::vector<int> temp_lb(possible.size(),-1);
    cash_low_bound = temp_lb;
    //DEBUGING
  ...
  void min_external_path_length(long int& external_path_length, std::vector<std::vector <int> >& t_possible, std::vector<std::vector<int> >& attempted, std::vector<int>& attempt){
    ...
    if(external_path_length + lower_bound_est(t_possible.size()) - t_possible.size()>= max){
  ...
  int lower_bound_est(int no_possiblities){
    if(cash_low_bound[no_possiblities] != -1){
      return cash_low_bound[no_possiblities];
    }else{
        if(no_possiblities == 1){
        cash_low_bound[1] = 1;
        return 1;
      }
      int estimation = 1;
      int height = 1;
      while(estimation <= no_possiblities){
        estimation *= (no_of_partitions);
        estimation += 1;
        height += 1;
      }
      int sum = 1;
      for(int k = 1; k < height; k++){
        sum += (k+1) * pow(no_of_partitions,k);
      }
      sum -= (height)*(estimation - no_possiblities);
      cash_low_bound[ no_possiblities ] = sum;
      return sum;
    }
  }
